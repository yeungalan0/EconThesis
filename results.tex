...
\tablelabel{tab:2sc}, \tablelabel{tab:3sc}, and \tablelabel{tab:4sc} show the relevant results\footnote{TODO: The results of the regression for all the variables can be found in Appendix A.} of the two stage selection model (outlined in section~\ref{methods}) run on the middle achievers, where the selection equation \eqref{eq:1} is a two category ordered probit, three category ordered probit, and four category ordered probit respectively. 
...
In the best cases the number of students who did not select into their first choice is about one half of the number of students who did select into their first choice.\footnote{TODO: See Table X and \sectlabel{data} for more details} 
...
In order to correct for selection bias we use the two stage selection model outlined in \sectlabel{methods:tssmodel}, and after comparing our results to the foundational OLS model outlined in \eqref{eq:0}, the overall results are the same.\footnote{TODO: See Appendix X for the regular OLS results.} 
...
Suppose that the grades of high achievers benefit from middle and low achievers, while the grade of low achievers are harmed by middle and high achievers,\footnote{TODO: Which is what Appendix X suggests to be the case.} then a trade off will need to be made if a policy is to be implemented. 
