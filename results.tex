The results suggest that the proportion of high achievers in a class has a negative impact on the grades of middle achievers, while the proportion of low achievers in a class has a positive impact on the grades of middle achievers\footnote{See \tablelabel{tab:2sc}, \tablelabel{tab:3sc}, and \tablelabel{tab:4sc}}.
...
\tablelabel{tab:2sc}, \tablelabel{tab:3sc}, and \tablelabel{tab:4sc} show the relevant results\footnote{TODO: The results of the regression for all the variables can be found in Appendix A.} of the two stage selection model\footnote{Outlined in \sectlabel{methods}} run on the middle achievers, where the selection equation \eqref{eq:1} is a two category ordered probit, three category ordered probit, and four category ordered probit respectively. 
...
%TODO: Move page number down on this table page
\begin{table}[htb]
  \centering
  \begin{threeparttable}
...
Our results run counter to those found in the current literature, granted that the current literature is rather scarce especially in the area of higher education \citep{kang2007classroom,carman2012classroom,burke2013classroom,schlosser2008inside,lavy2012good}. 
...
In the best cases the number of people who did not select into their first choice is about one half of the number of people who did select into their first choice.\footnote{TODO: See Table X and \sectlabel{data} for more details} 
...
It is important to note that when comparing our results from the two stage selection model to the OLS model outlined in \eqref{eq:0}, the overall results are the same.\footnote{TODO: See Appendix X for the regular OLS results.} 
The proportion of high achievers in a class has a negative impact on middle achieving student grades, and the proportion of low achievers in a class has a positive impact on middle achieving student grades.
This suggests that selection bias is most likely not affecting our sample to any significant degree. However, selection bias may still be a concern with any future studies that utilize a different sample.
...
If, on the other hand, the grades of low achievers were positively impacted by the proportion of middle achievers in the class, then by grouping low achievers and middle achievers the grades of both may be maximized.
...
Also, due to the time span of the data, we were unable to determine whether peer effects, grade curving, or both were responsible for our results. 
