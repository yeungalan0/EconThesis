...
In order to correct for selection bias we use the two stage selection model outlined in \sectlabel{methods:tssmodel}, and after comparing our results to the foundational OLS model outlined in \eqref{eq:0}, the overall results are the same.\footnote{TODO: See \appendixlabel{appendix:c} for the regular OLS results.} 
...
Suppose that the grades of high achievers benefit from middle and low achievers, while the grades of low achievers are harmed by middle and high achievers,\footnote{TODO: Which is what \appendixlabel{appendix:c} suggests to be the case.} then a trade off will need to be made if a policy is to be implemented. 
